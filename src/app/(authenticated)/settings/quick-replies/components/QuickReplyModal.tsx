...
import { Image as ImageIcon, VideoCamera, FileAudio, FileText, Trash, Plus, ArrowUp, ArrowDown, Clock, Stack, MagnifyingGlass } from '@phosphor-icons/react'
...
export default function QuickReplyModal({ scope, quickReply, existingCategories, allQuickReplies = [], onSave, onClose }: QuickReplyModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center p-0 md:p-4" onClick={onClose}>
      <div
        className={`app-safe-bottom sheet-enter bg-white rounded-t-2xl md:rounded-2xl shadow-xl w-full ${mode === 'sequence' ? 'max-w-2xl' : 'max-w-lg'} max-h-[90dvh] overflow-y-auto transition-[max-width]`}
