...
// A small Custom Switch component internally to avoid installing a heavy radical UI library if not needed yet
const CustomSwitch = ({ checked, onChange, disabled }: { checked: boolean, onChange: () => void, disabled: boolean }) => (
    <button
        ...
        disabled={disabled}
        className={`
            relative inline-flex h-6 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2
    ...
    >
        <span
            aria-hidden="true"
            className={`
                pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition-transform duration-300
...
const PermissionRow = ({
    ...
    title: string,
    description: string,
    module: keyof PermissionsSchema,
    action: string,
    permissions: PermissionsSchema,
    isAdmin: boolean,
    handleToggle: (module: keyof PermissionsSchema, action: string) => void,
    forceDisabled?: boolean,
    forceChecked?: boolean
}) => {
    // Evaluate effective check visually: admin overrides to ALWAYS true visually, disabled.
    // @ts-ignore
...
export default function RolePermissionsPanel({ organizationId, selectedRoleId }: RolePermissionsPanelProps) {
    ...
    useEffect(() => {
        async function fetchRolePermissions() {
            ...
            try {
                ...
                // Merge existing DB permissions with our default schema shape
    ...
    // Prevent rendering the Master role entirely to avoid any accidental edits 
    // since it's a global system role, not a local organizational role.
    ...
    const handleToggle = (module: keyof PermissionsSchema, action: string) => {
        if (isAdmin) return // Admins cannot be toggled

        setPermissions(prev => {
            const next = { ...prev }
            // Deep copy the nested module to ensure React properly detects the object reference change
            // @ts-ignore
    ...
    if (isLoading) {
        return (
            <div className="h-64 flex items-center justify-center bg-gray-50/50 rounded-xl border border-dashed border-gray-200">
                <LoadingSpinner text="Carregando permissões..." />
            </div>
        )
    }
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div className="px-6">
                        <PermissionRow
                            title="Ver apenas seus próprios leads"
                            description="Limita a visão do CRM apenas aos cards atribuídos a este usuário."
                            module="leads" action="view_own_only"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                        <PermissionRow
                            title="Criar e editar leads"
                            description="Permite adicionar novos contatos ou modificar existentes."
                            module="leads" action="create_edit"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                        <PermissionRow
                            title="Exportar dados"
                            description="Habilita o download de planilhas CSV/XLSX."
                ...
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div className="px-6">
                        <PermissionRow
                            title="Gerenciar Negócios"
                            description="Mover cards no Kanban, ganhar ou perder negociações."
                            module="pipeline" action="manage_deals"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                        <PermissionRow
                            title="Configurar Funis"
                            description="Adicionar, editar ou remover etapas do processo comercial."
                            module="pipeline" action="configure_funnels"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                    </div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div className="px-6">
                        <PermissionRow
                            ...
                            description="Convidar novos usuários e alterar permissões da organização."
                            module="settings" action="manage_members"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                        <PermissionRow
                            title="Visualizar Dashboard"
                            description="Permite o acesso à tela inicial de métricas e resumo do sistema."
                            module="settings" action="view_dashboard"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                        <PermissionRow
                            title="Visualizar Pipeline"
                            description="Permite o acesso ao módulo e tela de Negócios e Pipeline."
                            module="settings" action="view_pipeline"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                        <PermissionRow
                            title="Visualizar Chat"
                            description="Permite o acesso ao módulo e tela de Chat / WhatsApp."
                            module="settings" action="view_chat"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                        <PermissionRow
                            title="Visualizar Leads"
                            description="Permite o acesso ao módulo central de Leads e Contatos."
                            module="settings" action="view_leads"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                        <PermissionRow
                            title="Visualizar Configurações"
                            description="Permite o acesso à esta tela de Configurações."
                            module="settings" action="view_settings"
                            permissions={permissions} isAdmin={isAdmin} handleToggle={handleToggle}
                        />
                    </div>
                </div>
                ...
                {!isAdmin && (
                    <div className="border border-red-200 border-dashed bg-red-50/50 rounded-xl p-6 flex items-center justify-between">
                        ...
                        <button className="px-4 py-2 bg-white border border-red-200 text-red-600 font-semibold text-sm rounded-lg hover:bg-red-50 transition-colors shadow-sm">
                            Remover Cargo
                        </button>
                    </div>
                )}
                ...
                <div className="pt-4 flex justify-end">
                    {!isAdmin ? (
                        <button
                            ...
                            className={`px-5 py-2.5 bg-blue-600 text-white font-semibold text-sm rounded-lg shadow-sm transition-colors flex items-center gap-2
                                ${!hasChanges || isSaving ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'}
                            `}
                        >
                            {isSaving && <SpinnerGap className="w-4 h-4 animate-spin" weight="bold" />}
                            Salvar Alterações
                        </button>
                    ) : (
                        <div className="px-4 py-2 bg-gray-100 rounded-lg text-sm text-gray-500 font-medium">
                            Cargo administrador fixo. Permissões não podem ser removidas.
                        </div>
                    )}
                </div>
