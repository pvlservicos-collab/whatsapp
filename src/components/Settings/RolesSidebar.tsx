...
export default function RolesSidebar({ organizationId, selectedRoleId, onSelectRole }: RolesSidebarProps) {
    ...
    useEffect(() => {
        async function fetchRoles() {
            try {
                ...
                // Select the first valid role by default if none is selected
                if (!selectedRoleId && validRoles.length > 0) {
                    onSelectRole(validRoles[0].id)
                }
    ...
    }, [organizationId, selectedRoleId, onSelectRole])
    ...
    const getRoleDescription = (name: string) => {
        switch (name.toLowerCase()) {
            ...
            case 'gerente':
                return 'Visão geral e gestão de equipe'
            case 'vendedor':
                return 'Gestão de leads e pipeline'
            case 'visualizador':
                return 'Apenas leitura de dados'
            default:
